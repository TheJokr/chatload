...
// Version
static const std::string CHATLOAD_VERSION = "1.0.0";


// POST Endpoint
// CHATLOAD_HOST = Base URI with protocol, hostname, and optionally a port number (eg. http://example.com)
static const std::wstring CHATLOAD_HOST = L"YOUR_HOST";
// CHATLOAD_RESOURCE = Path, Query, and Fragment of POST endpoint (eg. /exampleScript.php)
static const std::wstring CHATLOAD_RESOURCE = L"YOUR_RESOURCE";


// Returns a std::wstring with username
...
int main(int argc, char* argv[]) {
	...
	if (argc == 2 && (std::strcmp(argv[1], "-v") == 0 || std::strcmp(argv[1], "--version") == 0)) {
		std::cout << argv[0] << " " << CHATLOAD_VERSION << std::endl;
	...
	web::http::client::http_client client(CHATLOAD_HOST);
	...
	pplx::task<web::http::http_response> postChars = client.request(web::http::methods::POST, CHATLOAD_RESOURCE, L"name=" + joinVec(charNames, L","), L"application/x-www-form-urlencoded");
